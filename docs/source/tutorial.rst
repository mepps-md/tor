...
Let's create a tender with the minimal (only required) data set:
...
We can see the same response we got after having created tender.
...
The previous tender contained only required fields. Let's try to create a tender with more data
...
Let's update a tender by supplementing it with all other essential properties:
...
You can upload contract documents. Let's upload a contract document:
