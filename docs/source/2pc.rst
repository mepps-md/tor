...
The 2-phase commit provides a mechanism for CDB to publish only the tenders that clients are able to control and duplicates of which they have a right to cancel.
...
Creating a tender with a single-phase commit
--------------------------------------------
...
A request `POST /tenders` creates a tender in status ``draft``. As a result, an ``acc_token`` is passed for further tender management. 
...
Repeating the request for publication in case of problem with receiving a response from the server will not cause errors.
